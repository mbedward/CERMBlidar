...
The function `prepare_tile` imports data from a LAS file and normalizes point heights to ground level.
...
rcounts <- get_strata_counts(las, StrataSpecht, res = 10)
plot(rcounts)
...
Mean cover estimates are derived from the rasters of point counts by stratum. If point counts are not needed for any other purpose, these two steps can be combined using the `%>%` operator from the dplyr package.
